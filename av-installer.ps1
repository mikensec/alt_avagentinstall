new-module -name install_agent -scriptblock {
    ...
    $SysmonConfigSrc = "https://raw.githubusercontent.com/mikensec/alt_avagentinstall/main/sysmon_config_schema4_0.xml"
    ...
    $OsquerySrc = "https://github.com/mikensec/alt_avagentinstall/raw/main/alienvault-agent-20.08.0003.0301.msi"
    $OsqueryDst = "$env:TEMP\alienvault-agent.msi"
    [bool] $IsSysmonDownloaded = $false
    [bool] $IsSysmonConfigDownloaded = $false
    [bool] $IsLegacyAgent = $true
    ...
    function Install-Project-Internal() {
        ...
        # Download Sysmon
        #===================================================
        if (($IsLegacyAgent) -and (-not $nosysmon)) {
        ...
        if ([string]::IsNullOrEmpty($hostid)) {
            $hostid="00000000-f56f-4821-8557-f62cda86c15a"
        }

        $output = "--tls_hostname=api.agent.alienvault.cloud/osquery-api/eu-central-1", "--host_identifier=specified", "--specified_identifier=$hostid"
